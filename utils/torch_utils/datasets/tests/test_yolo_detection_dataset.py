"""Check `YOLODetectionDataset`. Iterate over it and show images with bboxes.
...
from utils.torch_utils.datasets.yolo_detection_dataset import (
...
def main(
    ...
    for idx in tqdm(range(len(dataset)), 'Iterating over source samples'):
        ...
        # Draw bboxes
        bboxes = dataset.convert_bboxes_to_xyxy(bboxes, image.shape[:2])
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True,
                            collate_fn=YOLODetectionDataset.collate_func,
                            num_workers=0)
